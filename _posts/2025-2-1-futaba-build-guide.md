...
| \| | ビット交換式等、極端に短いドライバーだとネジに届かない場合があります。  |
...
キット付属のピンヘッダを写真のように切り分けます。  
ニッパーで切ってもいいですが、基板にピンヘッダを差し込んで手で折るのが簡単です。
...
初期状態では、トラックパッド上の真ん中のキーには `F5` キーを割り当てています。  
ブラウザ版のVialを使っている場合、このキーを押すとページの再読み込みが発生します。
ここでは動作確認したいだけなので、ページの再読み込みをもって入力できたと判断してもらって構いません。
...
**トラックパッドカバーに表裏はありませんが、各面で触り心地が異なります。**  
気に入った面を表側として扱ってください。

裏側になる面一面に両面テープを貼り付けます。両面テープは重ねず、全面に貼るようにしてください。  
隙間を開けるとそこだけ感度が変わる可能性があります。
